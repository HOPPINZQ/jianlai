package com.hoppinzq.service.dom;

/**
 * @author:ZhangQi
 **/
public class H1 extends BlockDom{

    private Object text;
    private int num=1;

    public void addValue(String value) {
        this.text = value;
    }

    public void setNum(int num) {
        this.num = num;
    }

    /**
     * @param text 内容
     */
    public H1(Object text) {
        super();
        this.text = text;
    }

    /**
     */
    public H1(Object text,int num) {
        super();
        this.text = text;
        this.num=num;
    }

    public H1() {
        super();
    }

    @Override
    public String toHtml() {
        StringBuilder sb = new StringBuilder("<h").append(this.num);
        sb.append(tagAttribute()).append(tagStyle()).append(">");
        sb.append(text);
        sb.append("</h").append(this.num).append(">");
        return sb.toString();
    }

    @Override
    public String toFormatHtml() {
        return this.toHtml();
    }

    @Override
    public void clearElement() {
        super.clearStyle();
    }

    @Override
    public HtmlTag style(String styleKey, String styleVal) {
        this.styleMap.put(styleKey, styleVal);
        return this;
    }

    @Override
    public String text() {
        return this.text.toString();
    }

}
