package com.hoppinzq.service.chat.common;
...
public class TokenHolder {
    ...
    private void parseJson(String result) throws DemoException {
        ...
        if (scope != null && (json.get("scope").toString().indexOf(scope)==-1)) {
            throw new DemoException("scope not exist, " + scope + "," + result);
        }
        token = json.get("access_token").toString();
        expiresAt = System.currentTimeMillis() + Long.parseLong(json.get("expires_in").toString()) * 1000;
    }
